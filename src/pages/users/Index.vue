<!--
  ...
  - Created on : Friday 31 Jul, 2020 04:18:25 BDT
  -->
            ...
            <q-fab-action :color="$colors.negative" class="small-action" glossy icon="delete" label-class="text-xx-small" square>
              <q-popup-proxy :breakpoint="700">
                <q-banner dense>
                  ...
                  Would you really like to delete `{{ row.firstName }}` for forever?
                  <template v-slot:action>
                    <q-btn :color="$colors.negative" @click="deleteUser(row._id)" glossy v-close-popup>Yes</q-btn>
      ...
      <template v-slot:top="{pagination}">
        ...
        <div class="q-table__control col justify-end">
          <div class="text-primary row text-small" v-if="dataTable.totalItems">
            Showing {{ pagination.rowsNumber > 0 ? 1 : 0 }} to {{
              pagination.rowsNumber <
              pagination.rowsPerPage ? pagination.rowsNumber : pagination.rowsPerPage
            }} of
...
export default class Index extends Vue {
  ...
  mounted() {
    this.$root.$on('newUserDone', () => {
      this.loadTable()
    })
    this.$root.$on('editUserDone', () => {
      this.loadTable()
    })
  }
  ...
  onEnter() {
    this.requestColumns = ['image', 'firstName', 'lastName', 'gender', 'email', 'phone', 'type', 'ip', 'addedBy']
    this.requestColumns.forEach(each => {
      this.columns.push({
        label: this.$common.capitalizeFirstLetter(each),
        name: each,
        field: each,
        align: 'left',
        format: (val: any) => val,
  ...
  exportTable() {
    ...
    exportFile('users' + date.formatDate(new Date(), "-YYYY-MM-DD") + '.xlsx',
